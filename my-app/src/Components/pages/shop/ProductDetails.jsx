...
export const ProductDetails = () => {
  ...
  const fetchProduct = async () => {
    try {
      const response = await axios.get(`https://flask-app:8000/products/${id}`);
  ...
  const fetchImage = async (imageName) => {
    try {
      const response = await axios.get(
        `https://flask-app:8000/uploads/${imageName}`
      );
      setImageURL(`https://flask-app:8000/uploads/${imageName}`);
    } catch (error) {
